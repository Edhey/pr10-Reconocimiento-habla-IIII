...
namespace Whisper.Samples {
    ...
    /// </summary>
    public class MicrophoneWithCommands : MonoBehaviour {
        ...
        private void ProcessVoiceCommands(string text) {
            if (string.IsNullOrEmpty(text) || voiceCommands == null)
                return;

            foreach (var command in voiceCommands) {
                if (command != null && command.Matches(text)) {
                    command.Execute(commandTarget);
                    break; // Solo ejecutar el primer comando que coincida
                }
            }
        }
